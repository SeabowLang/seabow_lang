...
SBW_ValueLong::SBW_ValueLong(sbw_long val)
{
    this->value = (sbw_long*)malloc(sizeof(sbw_long));
    (*this->value) = val;
}
...
SBW_ValueLong::SBW_ValueLong(sbw_long *val)
{
    if (val)
    {
        ...
        this->value = (sbw_long*)0;
}

SBW_Value *SBW_ValueLong::AutoConvert(sbw_value_type dest_type)
{
    if (this->value)
    {
        switch (dest_type)
        {
            ...
            case VT_ANY_: return new SBW_ValueAny(new SBW_ValueLong(*this->value));
    ...
    else
    {
        switch (dest_type)
        {
            ...
            case VT_ANY_: return new SBW_ValueAny(new SBW_ValueLong((sbw_long*)0));
            default: return this->AutoConvertionError(dest_type);
        }
    }    
}

SBW_Value *SBW_ValueLong::operator+(sbw_none) { if (!this->value) return this->OpWithNullError(L"left +"); return new SBW_ValueLong(*this->value); }

SBW_Value *SBW_ValueLong::operator+(SBW_Value *val)
{
    sbw_long v = this->value ? *this->value : 0;
...
SBW_Value *SBW_ValueLong::operator++(sbw_none)
{
...
SBW_Value *SBW_ValueLong::operator++(int)
{
    if (!this->value) return this->OpWithNullError(L"right ++");
    SBW_ValueLong *old = new SBW_ValueLong((*this->value));
    (*this->value)++;
    return old;
}
...
SBW_Value *SBW_ValueLong::operator_power(SBW_Value *val)
{
    sbw_long v = this->value ? *this->value : 0;
...
SBW_Value *SBW_ValueLong::operator_power_eq(SBW_Value *val)
{
    if (!this->value)
        return this->OpWithNullError(L"**=");
...
SBW_Value *SBW_ValueLong::operator~(sbw_none) { if (!this->value) return this->OpWithNullError(L"~"); return new SBW_ValueLong(~(*this->value)); }
...
SBW_Value *SBW_ValueLong::operator!(sbw_none) { if (this->value) return new SBW_ValueBoolean((*this->value) == 0); return new SBW_ValueBoolean(true); }
...
SBW_Value *SBW_ValueLong::operator*(sbw_none) { return new SBW_ValuePointer(this, this->Type()); }

SBW_Value *SBW_ValueLong::operator_convert(sbw_value_type dest_type)
{
    if (this->value)
    {
        switch (dest_type)
        {
            ...
            case VT_TYPE_: return new SBW_ValueType(VT_LONG_);
...
SBW_Value *SBW_ValueLong::operator_abs(sbw_none) 
{ 
    if (this->value)
    {
        if (*this->value >= 0) return new SBW_ValueLong(*this->value);
        else return new SBW_ValueLong(-*this->value);
    } else return new SBW_ValueLong((sbw_long*)0);
}