...
SBW_ValueFloat::SBW_ValueFloat(sbw_float val)
{
    this->value = (sbw_float*)malloc(sizeof(sbw_float));
    (*this->value) = val;
}
...
SBW_ValueFloat::SBW_ValueFloat(sbw_float *val)
{
    if (val)
    {
        ...
        this->value = (sbw_float*)0;
}

SBW_Value *SBW_ValueFloat::AutoConvert(sbw_value_type dest_type)
{
    if (this->value)
    {
        switch (dest_type)
    ...
    else
    {
        switch (dest_type)
        {
            ...
            case VT_ANY_: return new SBW_ValueAny(new SBW_ValueFloat((sbw_float*)0));
            default: return this->AutoConvertionError(dest_type);
        }
    }    
}

SBW_Value *SBW_ValueFloat::operator+(sbw_none) { if (!this->value) return this->OpWithNullError(L"left +"); return new SBW_ValueFloat(*this->value); }

SBW_Value *SBW_ValueFloat::operator+(SBW_Value *val)
{
    sbw_float v = this->value ? *this->value : 0;
...
SBW_Value *SBW_ValueFloat::operator++(sbw_none)
{
...
SBW_Value *SBW_ValueFloat::operator++(int)
{
    if (!this->value) return this->OpWithNullError(L"right ++");
    SBW_ValueFloat *old = new SBW_ValueFloat((*this->value));
    (*this->value)++;
    return old;
}
...
SBW_Value *SBW_ValueFloat::operator_power(SBW_Value *val)
{
    sbw_float v = this->value ? *this->value : 0;
...
SBW_Value *SBW_ValueFloat::operator_power_eq(SBW_Value *val)
{
    if (!this->value)
        return this->OpWithNullError(L"**=");
...
SBW_Value *SBW_ValueFloat::operator!(sbw_none) { if (this->value) return new SBW_ValueBoolean((*this->value) == 0); return new SBW_ValueBoolean(true); }
...
SBW_Value *SBW_ValueFloat::operator*(sbw_none) { return new SBW_ValuePointer(this, this->Type()); }

SBW_Value *SBW_ValueFloat::operator_convert(sbw_value_type dest_type)
{
    if (this->value)
    {
        switch (dest_type)
        {
            ...
            case VT_STRING_: {
                ...
                stream << std::fixed << std::setprecision(7) << *this->value;
...
SBW_Value *SBW_ValueFloat::operator_abs(sbw_none) 
{
    if (this->value)
    {
        if (*this->value >= 0) return new SBW_ValueFloat(*this->value);
        else return new SBW_ValueFloat(-*this->value);
    } else return new SBW_ValueFloat((sbw_float*)0);
}