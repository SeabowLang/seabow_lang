...
SBW_ValueDouble::SBW_ValueDouble(sbw_double val)
{
    this->value = (sbw_double*)malloc(sizeof(sbw_double));
    (*this->value) = val;
}
...
SBW_ValueDouble::SBW_ValueDouble(sbw_double *val)
{
    if (val)
    {
        ...
        this->value = (sbw_double*)0;
}

SBW_Value *SBW_ValueDouble::AutoConvert(sbw_value_type dest_type)
{
    if (this->value)
    {
        switch (dest_type)
        {
            ...
            case VT_ANY_: return new SBW_ValueAny(new SBW_ValueDouble(*this->value));
...
SBW_Value *SBW_ValueDouble::operator+(sbw_none) { if (!this->value) return this->OpWithNullError(L"left +"); return new SBW_ValueDouble(*this->value); }

SBW_Value *SBW_ValueDouble::operator+(SBW_Value *val)
{
    sbw_double v = this->value ? *this->value : 0;
...
SBW_Value *SBW_ValueDouble::operator++(sbw_none)
{
...
SBW_Value *SBW_ValueDouble::operator++(int)
{
    if (!this->value) return this->OpWithNullError(L"right ++");
    SBW_ValueDouble *old = new SBW_ValueDouble((*this->value));
    (*this->value)++;
    return old;
}
...
SBW_Value *SBW_ValueDouble::operator_power(SBW_Value *val)
{
    sbw_double v = this->value ? *this->value : 0;
...
SBW_Value *SBW_ValueDouble::operator_power_eq(SBW_Value *val)
{
    if (!this->value)
        return this->OpWithNullError(L"**=");
...
SBW_Value *SBW_ValueDouble::operator!(sbw_none) { if (this->value) return new SBW_ValueBoolean((*this->value) == 0); return new SBW_ValueBoolean(true); }
...
SBW_Value *SBW_ValueDouble::operator*(sbw_none) { return new SBW_ValuePointer(this, this->Type()); }

SBW_Value *SBW_ValueDouble::operator_convert(sbw_value_type dest_type)
{
    if (this->value)
    {
        switch (dest_type)
...
SBW_Value *SBW_ValueDouble::operator_abs(sbw_none) 
{
    if (this->value)
    {
        if (*this->value >= 0) return new SBW_ValueDouble(*this->value);
        else return new SBW_ValueDouble(-*this->value);
    } else return new SBW_ValueDouble((sbw_double*)0);
}